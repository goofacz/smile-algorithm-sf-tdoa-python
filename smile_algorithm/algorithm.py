...
class Algorithm(salgorithm.Algorithm):
    ...
    def run_offline(self, directory_path):
        ...
        anchors = Anchors(os.path.join(directory_path, 'sf_tdoa_anchors.csv'))
        mobiles = Nodes(os.path.join(directory_path, 'sf_tdoa_mobiles.csv'))
        frames = Frames(os.path.join(directory_path, 'sf_tdoa_mobile_frames.csv'))

        results = []
        for mobile_node in mobiles:
            results += self._localize_mobile(mobile_node, anchors, frames)
    ...
    def _localize_mobile(self, mobile_node, anchors, frames):
        ...
        mobile_frames = frames[frames.node_mac_address == mobile_node["mac_address"], :]
        ...
        for i in range(sequence_numbers.size):
            ...
            result.mac_address = mobile_node["mac_address"]
            result.begin_true_position_x = current_beacons[0, "begin_true_position_x"]
            result.begin_true_position_y = current_beacons[0, "begin_true_position_y"]
            result.begin_true_position_z = current_beacons[0, "begin_true_position_z"]
            result.end_true_position_x = current_beacons[0, "end_true_position_x"]
            result.end_true_position_y = current_beacons[0, "end_true_position_y"]
            result.end_true_position_z = current_beacons[0, "end_true_position_z"]
